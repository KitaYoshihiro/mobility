...
class BBoxUtility(object):
    ...
    def iou(self, box):
        ...
        inter_left = np.maximum(self.priors[:, 0], box[0])
        inter_right = np.minimum(self.priors[:, 1], box[1])
    ...
    def encode_box(self, box, return_iou=True):
        ...
        encoded_box = np.zeros((self.num_priors, 2 + return_iou))
        ...
        #                                 assigned_priors[:, 2:4])
        assigned_priors_center = 0.5 * (assigned_priors[:, 0] +
                                        assigned_priors[:, 1])
        # assigned_priors_wh = (assigned_priors[:, 2:4] -
        #                       assigned_priors[:, :2])
        assigned_priors_w = (assigned_priors[:, 1] -
                              assigned_priors[:, 0])
        # we encode variance
        # encoded_box[:, :2][assign_mask] = box_center - assigned_priors_center
        encoded_box[:, 0][assign_mask] = box_center - assigned_priors_center
        # encoded_box[:, :2][assign_mask] /= assigned_priors_wh
        encoded_box[:, 0][assign_mask] /= assigned_priors_w
        # encoded_box[:, :2][assign_mask] /= assigned_priors[:, -4:-2]
        encoded_box[:, 0][assign_mask] /= assigned_priors[:, -2]
        # encoded_box[:, 2:4][assign_mask] = np.log(box_wh /
        #                                           assigned_priors_wh) #こちらはスケール（priorと正解boxの縦横比率の違いをエンコード）
        encoded_box[:, 1][assign_mask] = np.log(box_w /
                                                  assigned_priors_w) #こちらはスケール（priorと正解boxの縦横比率の違いをエンコード）
        # encoded_box[:, 2:4][assign_mask] /= assigned_priors[:, -2:]
        encoded_box[:, 1][assign_mask] /= assigned_priors[:, -1]
    ...
    def assign_boxes(self, boxes):
        ...
        assignment = np.zeros((self.num_priors, 2 + self.num_classes + 4)) # num_classesはバックグラウンドを含む（21）。最後の８桁はアサイン時（ここ）にはprediction時と異なる使い方。エンコード対象の（positiveの）priorには-8の桁に1が入る-7以降はゼロ固定
        # assignment[:, 4] = 1.0 # とりあえず全てのpriorをclass0、すなわちバックグラウンドに紐付ける
        assignment[:, 2] = 1.0 # とりあえず全てのpriorをclass0、すなわちバックグラウンドに紐付ける
        if len(boxes) == 0:
            return assignment
        # encoded_boxes = np.apply_along_axis(self.encode_box, 1, boxes[:, :4]) # 正解boxを
        encoded_boxes = np.apply_along_axis(self.encode_box, 1, boxes[:, :2]) # 正解boxを
        # encoded_boxes = encoded_boxes.reshape(-1, self.num_priors, 5)
        encoded_boxes = encoded_boxes.reshape(-1, self.num_priors, 3)
        ...
        assignment[:, :2][best_iou_mask] = encoded_boxes[best_iou_idx,
                                                         np.arange(assign_num),
                                                         :2] # best_iou_idxは複数boxのうちのどれが高いiouを持っているかのインデクスリスト／ndarrayのマージの仕方に慣れる！
        # assignment[:, 4][best_iou_mask] = 0
        assignment[:, 2][best_iou_mask] = 0
        # assignment[:, 5:-8][best_iou_mask] = boxes[best_iou_idx, 4:]
        assignment[:, 3:-4][best_iou_mask] = boxes[best_iou_idx, 2:]
        # assignment[:, -8][best_iou_mask] = 1 # ここでポジティブなアサインメントだけ1を代入しておく（フラグ）
        assignment[:, -4][best_iou_mask] = 1 # ここでポジティブなアサインメントだけ1を代入しておく（フラグ）
        return assignment
    ...
    def decode_boxes(self, mbox_loc, mbox_priorbox, variances):
        ...
        prior_width = mbox_priorbox[:, 1] - mbox_priorbox[:, 0]
        # prior_height = mbox_priorbox[:, 3] - mbox_priorbox[:, 1]
        # prior_center_x = 0.5 * (mbox_priorbox[:, 2] + mbox_priorbox[:, 0])
        # prior_center_y = 0.5 * (mbox_priorbox[:, 3] + mbox_priorbox[:, 1])
        prior_center = 0.5 * (mbox_priorbox[:, 1] + mbox_priorbox[:, 0])
        # decode_bbox_center_x = mbox_loc[:, 0] * prior_width * variances[:, 0]
        decode_bbox_center = mbox_loc[:, 0] * prior_width * variances[:, 0]
        # decode_bbox_center_x += prior_center_x
        decode_bbox_center += prior_center
        # decode_bbox_center_y = mbox_loc[:, 1] * prior_width * variances[:, 1]
        # decode_bbox_center_y += prior_center_y
        # decode_bbox_width = np.exp(mbox_loc[:, 2] * variances[:, 2])
        decode_bbox_width = np.exp(mbox_loc[:, 1] * variances[:, 1])
        # decode_bbox_width *= prior_width
        decode_bbox_width *= prior_width
        # decode_bbox_height = np.exp(mbox_loc[:, 3] * variances[:, 3])
        # decode_bbox_height *= prior_height
        # decode_bbox_xmin = decode_bbox_center_x - 0.5 * decode_bbox_width
        decode_bbox_xmin = decode_bbox_center - 0.5 * decode_bbox_width
        # decode_bbox_ymin = decode_bbox_center_y - 0.5 * decode_bbox_height
        # decode_bbox_xmax = decode_bbox_center_x + 0.5 * decode_bbox_width
        decode_bbox_xmax = decode_bbox_center + 0.5 * decode_bbox_width
        # decode_bbox_ymax = decode_bbox_center_y + 0.5 * decode_bbox_height
        # decode_bbox = np.concatenate((decode_bbox_xmin[:, None],
        #                               decode_bbox_ymin[:, None],
        #                               decode_bbox_xmax[:, None],
        #                               decode_bbox_ymax[:, None]), axis=-1)
        decode_bbox = np.concatenate((decode_bbox_xmin[:, None], decode_bbox_xmax[:, None]), axis=-1)
    ...
    def detection_out(self, predictions, background_label_id=0, keep_top_k=200,
        ...
        mbox_loc = predictions[:, :, :2]
        # variances = predictions[:, :, -4:]
        variances = predictions[:, :, -2:]
        # mbox_priorbox = predictions[:, :, -8:-4]
        mbox_priorbox = predictions[:, :, -4:-2]
        # mbox_conf = predictions[:, :, 4:-8]
        mbox_conf = predictions[:, :, 2:-4]
        results = []
